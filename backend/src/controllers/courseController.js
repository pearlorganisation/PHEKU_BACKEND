...
export const createCourse = asyncHandler(async (req, res,next) => {
    ...
    if (courseExists) {
      return next( new ApiError("Course already exists in the current university",400))
    }
...
export const getAllCourse =  asyncHandler(async(req,res,next)=>{
    ...
    return res.status(200).json(new ApiResponse("Fetched all the courses", course, 200))
})
...
export const getCourseById = asyncHandler(async(req,res,next)=>{
    ...
    }return res.status(200).json(new ApiResponse("Fetched the course",course,200));
})
...
export const deleteById= asyncHandler(async(req,res,next)=>{
    ...
    .status(200)
    .json(new ApiResponse("Successfully removed the course",null, 200))
})
...
console.log(courseId)
  const { title, duration, courseLevel, examType, fees, location, specialization } = req.body;
console.log( title )
...
}return res.json(new ApiResponse("Successfully updated the course ",updatedCourse,200));
})